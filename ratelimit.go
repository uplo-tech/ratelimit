...
type (
	...
	// rlStream is a rate-limiting wrapper for the uplomux.Stream interface.
	rlStream struct {
		uplomux.Stream
		rlrw rlReadWriter
	}
...
func NewRLStream(stream uplomux.Stream, rl *RateLimit, cancel <-chan struct{}) uplomux.Stream {
	return &rlStream{
		Stream: stream,
		rlrw: rlReadWriter{
			ReadWriter: stream,
